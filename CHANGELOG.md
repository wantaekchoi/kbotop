...
### Fixed
- **네트워크가 끊겼을 때 화면이 계속 "불러오는 중"이라고 말하던 것을 고쳤습니다.** 같은 화면의 아래 줄은 이미 오류를 알리고 있는데 본문만 60초가 지나도 기다리는 중이라고 했습니다 — 첫 응답이 오기 전까지는 실패해도 "아직 안 왔다"로만 보였기 때문입니다. 이제 실패를 아는 순간부터 본문이 그 이유를 보여줍니다. 경기 목록과 순위 둘 다입니다.
- **설정 파일이 깨졌을 때 어디를 고쳐야 하는지가 화면에서 사라지던 것을 고쳤습니다.** 아래 줄은 폭의 절반까지만 쓰기 때문에 80·100칸에서는 `config.toml could not be read, using …`에서 잘려, 정작 필요한 줄·열 번호가 220칸에서만 보였습니다. 이제 본문이 그 전문을 접어서 보여줍니다.
- **오류가 뜨면 아래 줄의 키 안내가 통째로 사라지던 것을 고쳤습니다.** 요청 주소 한 줄이 폭을 다 먹어서, 하필 나가는 법(`q`)이 필요한 순간에 그게 화면에서 사라졌습니다. 정작 원인은 220칸에서도 잘렸습니다. 이제 오류는 아래 줄의 절반까지만 쓰고, 원인을 앞에 두고 주소는 호스트만 남깁니다.
- **날짜 선택 창(`F2`)이 오늘 ±3일을 벗어나지 못하던 것을 고쳤습니다.** -3일로 옮겨도 다시 열면 또 오늘이 기준이라 거기서 더 갈 수 없었습니다. `--date`로는 아무 날짜나 볼 수 있는데 앱 안에서만 막혀 있었습니다. 이제 보고 있는 날짜가 기준이라 계속 거슬러 갈 수 있고, 커서가 지금 보는 날에 놓여 열자마자 `Enter`를 눌러도 오늘로 튕기지 않습니다.
- **날짜 선택 창이 오늘·어제·내일을 실제 날짜에만 붙이고, 진짜 오늘로 돌아갈 줄을 함께 보여줍니다.** 기준을 보고 있는 날로 옮기면서 세 라벨이 그 기준을 따라가 `--date 2026-06-01`에서는 "오늘 2026-06-01"처럼 전부 어긋났고, 목록 어디에도 진짜 오늘이 없어 `+3일`을 스물여덟 번 눌러야 돌아왔습니다. 이제 그 줄의 날짜가 실제로 오늘·어제·내일일 때만 그렇게 부르고(아니면 `보는 날`·`-2일`처럼 기준에서 며칠인지로), 오늘이 목록 밖이면 둘째 줄에 오늘이 들어옵니다 — `↓` 한 번에 돌아옵니다.
- **구단 링크 창(`o`)에서 `q`가 아무 반응이 없던 것을 고쳤습니다.** 다른 여섯 창은 모두 `q`로 닫히는데 여기만 빠져 있었고, 닫히지도 끝나지도 않아 갇힌 것처럼 보였습니다.
- **`TZ` 환경변수를 반대 부호로 읽던 것을 고쳤습니다.** `TZ=UTC-9`는 UTC+9(한국)를 뜻하는데 UTC-9로 읽어, 시계와 경기 시각이 18시간 어긋났습니다. `TZ=Asia/Seoul`처럼 지역 이름을 준 경우도 조용히 무시하지 않고 그대로 씁니다.
- **`--tz -14:00`처럼 존재하지 않는 시간대를 받아 주던 것을 고쳤습니다.** 실제 시간대는 -12:00부터 +14:00까지입니다.
- **`--license`가 옆에 붙은 인자에 따라 다르게 동작하던 것을 고쳤습니다.** `--license --date zzz`는 오류로 끝나고 `--license --team zzz`는 고지를 출력했습니다. 이제 옆에 무엇이 오든 고지를 출력합니다.
- **`LC_MESSAGES`를 보지 않던 것을 고쳤습니다.** 언어는 `LC_ALL` → `LC_MESSAGES` → `LANG` 순으로 정해지는데 가운데가 빠져 있었습니다. 빈 값은 "설정하지 않음"으로 봅니다.
- **`--help`에 마크다운 기호와 한국어 메모가 그대로 찍히던 것을 고쳤습니다.** `--license` 설명에서만 나던 일입니다.

### Removed
- `BaseState::runner_count` — 부르는 곳이 테스트 하나뿐이었습니다(화면은 세 베이스를 직접 그립니다).
- `Config::save()` — 설정 파일 경로를 스스로 정하던 저장 함수입니다. 저장은 이제 경로를 받는 쪽(`save_to`)만 남고, 실제 경로는 앱이 한 곳에서 주입합니다. 그 전에는 F9 키 처리를 부르는 **테스트가 개발자의 진짜 `config.toml`을 덮어썼습니다.**
