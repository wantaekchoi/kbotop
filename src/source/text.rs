...
/// 뉴스 제목 상한(char). 화면 한 줄에 들어가는 것보다 넉넉하되, 오염된 피드가
/// 수백만 자를 보내 렌더를 멈추게 하지는 못하게 하는 값이다 — 상한이 없을 때
/// 기사 오버레이 한 프레임이 394ms 걸렸다(실측). 100ms마다 다시 그리므로
/// 그 상태로는 화면이 사실상 멈춘다.
pub(crate) const TITLE_CHARS: usize = 300;
