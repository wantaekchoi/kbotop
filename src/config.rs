...
pub struct Config {
    ...
    /// 마우스로 클릭·스크롤할지(v0.27). **기본값은 끔이다**(v0.30까지는 켬).
    ///
    /// 마우스 캡처를 켜면 터미널의 드래그 선택·복사가 앱으로 넘어온다 — 즉
    /// 사용자가 이미 갖고 있던 시스템 동작을 **말없이 뺏는다**. v0.27은
    /// "꺼 두면 아무도 기능을 발견하지 못한다"를 근거로 켬을 기본으로 뒀는데,
    /// 발견성을 위해 남의 복사·붙여넣기를 깨는 건 방향이 반대다(사용자 지적
    /// 2026-08-02: "시스템 동작을 가로채는 행위는 좋지 않다"). Shift+드래그
    /// 우회로가 있다는 것도 그 우회로를 **알아야** 성립하는 이야기다.
    /// 발견성은 `F9` 설정 항목과 도움말이 맡는다.
    pub mouse: bool,
}
...
mod tests {
    ...
    /// **마우스는 설정 없이 켜지지 않는다.** 마우스 캡처는 터미널의 드래그
    /// 선택·복사를 앱으로 가져가므로, 아무 설정도 안 한 사람의 시스템 동작을
    /// 뺏으면 안 된다(사용자 지적 2026-08-02). v0.30까지는 기본이 켬이었다.
    #[test]
    fn the_mouse_is_off_until_the_user_turns_it_on() {
